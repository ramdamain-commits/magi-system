...
MAGI System は、3つの価値観を持つ AI パネルに同じ問いを審議させ、重要な判断の blind spot を見つけるための実験的な意思決定 UI です。現在は 1 回の Gemini リクエストで 3 人格と合議をまとめて生成し、UI 側で段階表示することで、ゲーム性を保ったまま API 負荷を抑えています。
...
- モデルには 3 人格分と合議結果をまとめて返させ、UI はローカルで `B -> C -> M -> CONSENSUS` の順に開示する
- 出力は JSON 形式の短い構造化レスポンスに限定し、`理由` は最大 2 件、`承認条件` は最大 2 件に抑える
